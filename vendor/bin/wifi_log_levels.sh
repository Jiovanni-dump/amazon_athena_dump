...
{
    SCREEN_STATUS=`$GETPROP vendor.wifi.wlan0.screen_status`
...
function log_kdm_rssi_level
{
    ...
    if [ $CHIPNSS = "2" ]; then
        ...
        rssibar2=$RSSIBAR
        log_to_kdm_minerva "operation=RSSILevel;key=${CHANNEL};metadata=${rssibar} bar;metadata1=${rssibar2} bar;metadata2=${BANDWIDTH};metadata3=Data;metadata4=${SCREEN_STATUS}"
    else
        log_to_kdm_minerva "operation=RSSILevel;key=${CHANNEL};metadata=${rssibar} bar;metadata2=${BANDWIDTH};metadata3=Data;metadata4=${SCREEN_STATUS}"
    fi

    log_to_kdm_minerva "operation=RSSILevel;key=${CHANNEL};metadata=${beacon_rssibar} bar;metadata2=${BANDWIDTH};metadata3=Beacon;metadata4=${SCREEN_STATUS}"
}
...
function log_kdm_snr_level
{
    ...
    snr=$(($dataRssi - $maxNoise))
    kdm_snr_level ${snr}
    snrbar=$SNRBAR

    ## Dual-chain metrics are only enabled on Mantis
    if [ $CHIPNSS = "2" ]; then
        snr=$(($dataRssi - $maxNoise2))
        kdm_snr_level ${snr}
        snrbar2=$SNRBAR
        ...
        log_to_kdm_minerva "operation=SNRLevel;key=${CHANNEL};metadata=${snrbar} bar;metadata1=${rssibar} bar;metadata2=${mcs};metadata4=${SCREEN_STATUS}"
    fi
}
